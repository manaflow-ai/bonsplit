...
final class BonsplitTests: XCTestCase {
  ...
  private final class CustomActionDelegateSpy: BonsplitDelegate {
    ...
    func splitTabBar(
      _ controller: BonsplitController, didRequestCustomAction identifier: String,
      inPane pane: PaneID
    ) {
  ...
  private final class ObservationInvalidationFlag: Sendable {
  ...
  func testCustomSplitActionButtonSupportsImageDataIcon() throws {
    ...
    let decoded = try JSONDecoder().decode(
      BonsplitConfiguration.SplitActionButton.self, from: encoded)
  ...
  func testSplitActionSystemImageUsesFallbackForUnknownSymbols() {
    ...
    XCTAssertEqual(
      image,
      TabBarStyling.SplitActionSystemImage(
  ...
  func testMinimalModeDoesNotReserveHiddenSplitButtonStrip() {
    ...
    XCTAssertEqual(
      TabBarStyling.trailingTabContentInset(
        showSplitButtons: true, isMinimalMode: false, buttonCount: 2),
    ...
    XCTAssertEqual(
      TabBarStyling.trailingTabContentInset(
        showSplitButtons: true, isMinimalMode: false, buttonCount: 0),
      0,
  ...
  func testTabBarHitRegionRegistryIgnoresViewsHiddenByAncestors() {
    ...
    let tabBar = FakeTabBarHitRegionView(
      frame: NSRect(x: 32, y: contentView.bounds.maxY + 6, width: 180, height: 30))
  ...
  func testResolvedDividerThicknessClampsToRange() {
    ...
    XCTAssertEqual(
      TabBarMetrics.resolvedDividerThickness(-5), TabBarMetrics.minimumDividerThickness)
    XCTAssertEqual(
      TabBarMetrics.resolvedDividerThickness(999), TabBarMetrics.maximumDividerThickness)
  ...
  func testTabBarAndSplitButtonBackdropSurfacesCanBeExplicit() {
    ...
    let barColor = TabBarColors.nsColorBarBackground(for: appearance).usingColorSpace(.sRGB)!
    let backdropColor = TabBarColors.nsColorSplitButtonBackdropSurface(for: appearance)
  ...
  func testSharedBackdropUsesSemanticBackgroundForTextAndHover() {
    ...
    let hover = TabBarColors.nsColorHoveredTabBackground(for: appearance).usingColorSpace(.sRGB)!
  ...
  func testSharedBackdropActiveTabBackgroundIsClear() {
    ...
    let active = TabBarColors.nsColorActiveTabBackground(for: appearance).usingColorSpace(.sRGB)!
  ...
  func testSplitActionPressedStateUsesHigherContrast() {
    ...
    let idleIcon = TabBarColors.nsColorSplitActionIcon(for: appearance, isPressed: false)
      .usingColorSpace(.sRGB)!
    let pressedIcon = TabBarColors.nsColorSplitActionIcon(for: appearance, isPressed: true)
      .usingColorSpace(.sRGB)!
  ...
  func testSplitPaneWithOptionalTabPreservesCustomTitleFlag() {
    ...
    guard
      let newPaneId = controller.splitPane(
        sourcePaneId, orientation: .horizontal, withTab: customTab)
    else {
  ...
  func testCancelTabDragIgnoresStaleGeneration() {
    ...
    guard let secondPaneId = controller.rootNode.allPaneIds.first(where: { $0 != firstPane.id }),
      ...
      let secondTab = secondPane.selectedTab
    else {
  ...
  func testTabContextMenuBuilderCreatesAppKitMoveSubmenu() throws {
    ...
    XCTAssertEqual(
      moveItem?.submenu?.items.map(\.title), ["Move Tab to New Workspace", "Workspace A"])
  ...
  func testTabContextMenuBuilderCreatesForkConversationSubmenu() throws {
    ...
    let destinationItems = try XCTUnwrap(
      forkSubmenuItem.submenu?.items.filter { !$0.isSeparatorItem })
    XCTAssertEqual(
  ...
  @MainActor
  func testTabContextMenuBuilderKeepsForkConversationVisibleWhenOpenAvailabilityIsRefreshing()
    throws
  {
    let target = TabContextMenuActionTarget()
    let state = TabContextMenuState(
  ...
  func testMultipleTabsVisibilityHidesPaneBarUntilThereAreMultipleTabs() throws {
    XCTAssertFalse(
      try XCTUnwrap(renderedPaneContainerHasTabBar(tabCount: 0, visibility: .multipleTabs)))
    XCTAssertFalse(
      try XCTUnwrap(renderedPaneContainerHasTabBar(tabCount: 1, visibility: .multipleTabs)))
    XCTAssertTrue(
      try XCTUnwrap(renderedPaneContainerHasTabBar(tabCount: 2, visibility: .multipleTabs)))
  }
  ...
  func testResolvedFaviconImageKeepsExistingImageWhenIncomingDataIsInvalid() {
    ...
    let resolved = TabItemStyling.resolvedFaviconImage(
      existing: existing, incomingData: invalidData)
  ...
  func testLoadingSpinnerUsesCoreAnimationRotationLayer() throws {
    ...
    try withExtendedLifetime(window) {
      ...
      XCTAssertEqual(
        animation.duration, TabLoadingSpinnerLayerView.rotationDuration, accuracy: 0.001)
  ...
  func testTabControlShortcutHintPolicyMatchesConfiguredModifiers() {
    withShortcutHintDefaultsSuite { defaults in
      ...
      XCTAssertNil(
        TabControlShortcutHintPolicy.hintModifier(for: [.control, .shift], defaults: defaults))
      XCTAssertNil(
        TabControlShortcutHintPolicy.hintModifier(for: [.command, .option], defaults: defaults))
  ...
  func testFileURLPasteboardReaderReturnsFileURLs() throws {
    ...
    let pasteboard = NSPasteboard(
      name: NSPasteboard.Name("bonsplit.file-drop.\(UUID().uuidString)"))
  ...
  func testTabShortcutHintStyleMatchesCommandHintPillFont() {
    ...
    XCTAssertEqual(
      TabControlShortcutHintStyle.measurementFont.fontDescriptor.object(forKey: .face) as? String,
      "Semibold")
  }
  ...
  func testInactiveSelectedTabIndicatorUsesDesaturatedAccent() {
    guard let focusedSaturation = renderedTabBarIndicatorSaturation(isFocused: true),
      let unfocusedSaturation = renderedTabBarIndicatorSaturation(isFocused: false)
    else {
  ...
  func testFullWidthTabModeIndicatorUsesFocusedAccent() {
    guard let focusedSaturation = renderedFullWidthTabModeIndicatorSaturation(isFocused: true),
      let unfocusedSaturation = renderedFullWidthTabModeIndicatorSaturation(isFocused: false)
    else {
  ...
  func testSharedBackdropActionLaneSeparatorMatchesBackdropGradientGeometry() {
    ...
    XCTAssertEqual(
      snapshot.actionLaneSeparatorFadeWidth, snapshot.backdropFadeWidth, accuracy: 0.0001)
    XCTAssertEqual(
      snapshot.actionLaneSeparatorSolidWidth, snapshot.actionLaneWidth, accuracy: 0.0001)
  ...
  func testActionLaneFallbackSeparatorClipsToSelectedSeparatorGap() {
    ...
    XCTAssertNil(
      geometry.fallbackSeparatorMaskFrame(
        totalWidth: size.width,
        height: size.height,
  ...
  func testSplitContentAlphaMatchesSinglePane() {
    ...
    guard
      let singlePaneAlpha = renderedAlpha(
  ...
  func testTabBarDragZoneFocusesInactivePaneInMinimalMode() throws {
    ...
    XCTAssertTrue(
      focused, "Inactive-pane drag zone should focus the pane before starting a window drag")
    XCTAssertFalse(dragged, "Inactive-pane focus click should not immediately begin a window drag")
    XCTAssertFalse(
      view.mouseDownCanMoveWindow,
      "Inactive-pane drag zone should not advertise window dragging to AppKit")
  }
  ...
  func testTabBarDragZoneMinimalModeNeverRequestsNewTabAfterSingleThenDoubleClick() throws {
    ...
    XCTAssertFalse(
      requestedNewTab, "Minimal-mode drag zone double-clicks must not request new tabs")
    XCTAssertFalse(
      dragged, "A plain click followed by a double-click should not start a window drag")
  }
  ...
  func testTabBarDragZoneDoubleClickDoesNotRequestNewTabInMinimalMode() throws {
    ...
    XCTAssertFalse(
      requestedNewTab,
      "Minimal-mode drag zone double-click should behave like titlebar chrome, not new-tab chrome")
  ...
  func testTabBarDragZoneSingleClickDoesNotRequestNewTabInStandardMode() throws {
    ...
    XCTAssertEqual(
      newTabCount, 0,
      "Standard-mode drag zone single click should wait for a double-click before creating a tab")
  ...
  func testTabBarDragZoneSingleClickFocusesInactivePaneInStandardMode() throws {
    ...
    XCTAssertEqual(
      newTabCount, 0, "Standard-mode inactive-pane single click should not create a tab")
    XCTAssertFalse(
      dragged, "Standard-mode inactive-pane single click should not begin a window drag")
  }
  ...
  func testTabBarTrailingEmptyChromeCapturesOnlyEmptyArea() throws {
    ...
    XCTAssertNil(
      view.hitTest(NSPoint(x: 40, y: 15)), "The empty chrome catcher must not cover tabs")
    XCTAssertNil(
      view.hitTest(NSPoint(x: 300, y: 15)),
      "The empty chrome catcher must not cover the action button lane")
  ...
  func testTabBarDragZoneKeepsFocusedPaneWindowDragInMinimalMode() throws {
    ...
    XCTAssertTrue(
      dragged, "Focused-pane drag zone should continue to start window drags in minimal mode")
    XCTAssertFalse(
      view.mouseDownCanMoveWindow,
      "Focused-pane drag zone must not advertise window dragging to AppKit or AppKit steals mouseUp and breaks new-tab double-clicks"
    )
  }
  ...
  private func renderedAlpha(
    ...
    withNativeRenderer(controller: controller, size: size) { root in
      renderedColor(in: root, at: samplePoint)?.alphaComponent
    }
  }
  ...
  private func renderedTabBarValue<T>(
    ...
    resolvedAppearance.showSplitButtons = showSplitButtons
    let controller = BonsplitController(
      configuration: BonsplitConfiguration(
    ...
    controller.internalController.focusedPaneId = isFocused ? pane.id : nil

    let renderSize = size ?? NSSize(width: 160, height: TabBarMetrics.barHeight)
    return withNativeRenderer(controller: controller, size: renderSize, extract: extract)
  }
  ...
  private func renderedPaneContainerHasTabBar(
    ...
    guard let pane = controller.internalController.rootNode.allPanes.first else { return nil }
    let tabs = (0..<tabCount).map { TabItem(title: "Tab \($0 + 1)", icon: nil) }
    pane.tabs = tabs
    pane.selectedTabId = tabs.first?.id
    return withNativeRenderer(
      controller: controller,
      size: NSSize(width: 320, height: 180)
    ) { root in
      guard let tabBar = firstDescendant(ofType: BonsplitNativeTabBarView.self, in: root) else {
        return false
      }
      return !tabBar.isHidden && tabBar.frame.height > 0
    }
  }
  ...
  private func renderedFullWidthPaneChromeAlpha(
    ...
    return withNativeRenderer(
      ...
      return !tabBar.isHidden && tabBar.frame.height > 0 ? 1 : 0
    }
  }

  @MainActor
  private func withNativeRenderer<T>(
    ...
    let renderer = BonsplitViewController(
      controller: controller,
      content: { _, _ in Self.transparentViewController() },
      emptyPane: { _ in Self.transparentViewController() }
    )
    let window = NSWindow(
  ...
  private static func assertValidFrames(in view: NSView) {
    ...
    XCTAssertTrue(
      ...
      "Invalid native geometry for \(type(of: view)): frame=\(frame) bounds=\(bounds)"
    )
    for subview in view.subviews {
      assertValidFrames(in: subview)
    }
  }

  @MainActor
  private static func transparentViewController() -> NSViewController {
  ...
  @MainActor
  private func renderedSelectedIndicatorRangeAfterManualScroll(size: NSSize) -> ClosedRange<
    CGFloat
  >? {
    renderedTabBarValue(
      isFocused: true,
  ...
  private func renderedSplitButtonBackdropFadeVerticalColorDelta() -> CGFloat? {
    ...
    return renderedTabBarValue(
      ...
      guard
        let top = renderedColorInViewCoordinates(
          in: hostingView,
  ...
  private func renderedSplitButtonLaneSolidBackdropBrightness() -> CGFloat? {
    ...
    return renderedTabBarValue(
      ...
      let laneStartX = size.width - splitButtonLaneWidth
      guard
        let color = renderedColorInViewCoordinates(
          in: hostingView,
  ...
  private func renderedSplitButtonContentFadeStartBackdropBrightness() -> CGFloat? {
    ...
    let size = NSSize(width: 240, height: 28)
    let splitButtonLaneWidth = visibleSplitButtonLaneWidth(size: size, buttonCount: buttonCount)
    let contentFadeWidth = BonsplitConfiguration.Appearance.SplitButtonBackdropEffect.default
      .contentFadeWidth
    let appearance = BonsplitConfiguration.Appearance(
      tabBarHeight: size.height,
      splitButtonBackdropEffect: .default,
  ...
  @MainActor
  private func renderedSelectedIndicatorBackdropBrightnesses() -> (
    leading: CGFloat, trailing: CGFloat
  )? {
    let buttonCount = BonsplitConfiguration.SplitActionButton.defaults.count
    let size = NSSize(width: 240, height: 28)
    let splitButtonLaneWidth = visibleSplitButtonLaneWidth(size: size, buttonCount: buttonCount)
    let fadeWidth = BonsplitConfiguration.Appearance.SplitButtonBackdropEffect.default
      .contentFadeWidth
    let appearance = BonsplitConfiguration.Appearance(
      tabBarHeight: size.height,
      splitButtonBackdropEffect: .default,
    ...
    return renderedTabBarValue(
      ...
      let laneStartX = size.width - splitButtonLaneWidth
      guard
        let leading = renderedColorInViewCoordinates(
          in: hostingView,
  ...
  private func renderedSharedBackdropActionLaneBottomSeparatorAlphas() -> (
    ...
    return renderedTabBarValue(
      ...
      guard
        let fadeStart = renderedColorInViewCoordinates(
          in: hostingView,
  ...
  private func renderedSelectedTabLeftSeparatorAlphas() -> (top: CGFloat, bottom: CGFloat)? {
    ...
    return renderedTabBarValue(
      ...
      guard
        let top = renderedColorInViewCoordinates(in: hostingView, at: NSPoint(x: separatorX, y: 4))?
          .usingColorSpace(.sRGB)?
          .alphaComponent,
        let bottom = renderedColorInViewCoordinates(
          in: hostingView,
  ...
  private func makeMouseEvent(
    ...
    guard let window = view.window else {
      throw NSError(
        domain: "BonsplitTests", code: 1, userInfo: [NSLocalizedDescriptionKey: "Missing window"])
    }
    ...
    guard
      ...
      throw NSError(
        domain: "BonsplitTests", code: 2,
        userInfo: [NSLocalizedDescriptionKey: "Failed to create mouse event"])
    }
    return event
  }
